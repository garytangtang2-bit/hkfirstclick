/**
 * fixMissingLangs.mjs
 * Re-optimizes only languages where seo_title lacks power words/year.
 */
...
function isOptimized(title) {
  return /2026|Best|Top|Must|Hidden|Local|Gem|\d+/.test(title);
}
...
async function fixFile(filePath, type) {
  ...
  for (const [lang, langName] of Object.entries(LANG_NAMES)) {
    ...
    if (!t) continue;
    if (isOptimized(t.seo_title ?? '')) continue; // skip already optimized
...
async function main() {
  ...
  // Only process files that have unoptimized languages
  const toFix = allFiles.filter(({ filePath }) => {
    const data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
    return Object.keys(LANG_NAMES).some(lang => {
      const t = data.translations?.[lang];
      return t && !isOptimized(t.seo_title ?? '');
    });
  });

  console.log(`Fixing ${toFix.length} files with missing optimized SEO...`);
